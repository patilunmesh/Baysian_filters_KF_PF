...
class RobotGroundTruth:
    ...
    def set_move_left_probabilities(self, move_left=0.8, move_right=0.05):
        ...
        # # YOUR CODE HERE
        if 0 < move_left + move_right <= 1:
            self.move_probabilities["move_left"] = {"left": move_left, "stay_put": 1-move_left-move_right, "right": move_right,}
    ...
    def set_move_right_probabilities(self, move_left=0.05, move_right=0.8):
        ...
        if move_left + move_right <= 1:
            self.move_probabilities["move_right"] = {"right": move_right, "stay_put": 1-move_left-move_right, "left": move_left, }
    ...
    def set_move_continuos_probabilities(self, sigma=0.1):
        ...
        if sigma > 0:
            self.move_probabilities["move"] = {"move_continuous": sigma}
    ...
    def move_left(self, step_size):
        ...
        step_dir = 0
        k = self.move_probabilities["move_left"]
        for v in k.values():
            if v < 0.0 or v > 1.0:
                ValueError(f"Value {v} not between zero and one")    
        if not np.isclose(sum(k.values()), 1.0):
            ValueError(f"Sum of probabilities should be 1, is {sum(k.values())}")
        ...
        if back == "right" : step_dir = 1
        elif back == "left": step_dir = -1
    ...
    def move_right(self, step_size):
        ...
        step_dir = 0
        k = self.move_probabilities["move_right"]
        for v in k.values():
            if v < 0.0 or v > 1.0:
                ValueError(f"Value {v} not between zero and one")    
        if not np.isclose(sum(k.values()), 1.0):
...
# YOUR CODE HERE
        k = self.move_probabilities["move"]
        noisy_amount += np.random.normal(0, k["move_continuous"])
