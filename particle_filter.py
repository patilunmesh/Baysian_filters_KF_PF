...
# YOUR CODE HERE
        size = 1/(2*n_samples)
        self.particles = np.linspace(0, 1, n_samples) #+ size
        self.weights = (2*size)*np.ones((np.shape(self.particles)))
...
# YOUR CODE HERE
        ...
        k = robot_ground_truth.move_probabilities["move"]
        mu, sig = 0, k["move_continuous"]
        for i in range(len(self.particles)):
            noise = np.random.uniform(0, sig)
            self.particles[i] += (amount + noise)
            if self.particles[i] > 1:
                self.particles[i] = 1 - noise*0.1
            if self.particles[i] < 0:
                self.particles[i] = 0 + noise*0.1
    ...
    def calculate_weights_door_sensor_reading(self, world_ground_truth, robot_sensor, sensor_reading):
        ...
        sensordoor = robot_sensor.sensor_prob_dict["door"]
        sensornodoor = robot_sensor.sensor_prob_dict["no_door"]
# YOUR CODE HERE
        for i, p in enumerate(self.particles):
            px = self.weights[i]
            is_in_front_of_door = world_ground_truth.is_location_in_front_of_door(self.particles[i])
            if is_in_front_of_door:
                if sensor_reading:
                    pyx = sensordoor["see"]
                else: pyx = sensordoor["nosee"]
            else:
                if sensor_reading:
                    pyx = sensornodoor["see"]
                else: pyx = sensornodoor["nosee"]
            self.weights[i] = pyx * px
        summ = np.sum(np.array(self.weights))
        self.weights = self.weights / summ
...
# YOUR CODE HERE
        mu, sigma = robot_sensors.wall_sensor_probs["mean"], robot_sensors.wall_sensor_probs["sigma"]
        for i in range(len(self.particles)):
            diff = self.particles[i] - dist_reading
            self.weights[i] += gaussian(diff, mu, sigma)            
        summ = np.sum(np.array(self.weights))
        self.weights = self.weights / summ
...
# YOUR CODE HERE
        new_weights = np.cumsum(self.weights)
        new_weights /= np.sum(new_weights)
        zero_to_one = np.random.uniform(0,1)
        sampled_particles = []
        for i in range(len(self.particles)):
            if zero_to_one > new_weights[i]:
                sampled_particles.append(self.particles[i])
            else: break
        self.particles = sampled_particles
        self.weights = (1/len(self.particles))*np.ones((len(self.particles)))
